...
export const showIncomingCallDialog = (callType, acceptCallHandler, rejectCallHandler) => {
    ...
    const incomigCallDialog = elements.getIncommingCallDialog(callTypeInfo, acceptCallHandler, rejectCallHandler);

    const dialog = document.getElementById("dialog");
    dialog.querySelectorAll("*").forEach((dialog) => dialog.remove());

    dialog.appendChild(incomigCallDialog);
};
...
export const showInfoDialog = (preOfferAnswer) => {
    ...
    if(preOfferAnswer === constants.preOfferAnswer.CALLEE_NOT_FOUND) {
        infoDialog = elements.getInfoDialog("Callee not found", "Please check the personal code");
    }
...
export const updateUIAfterHangUp = (callType) => {
    ...
    // 把mic和camera 图片换成on的图片
    // 因为前面代码里面已经开了麦和localvideo了
...
export const updateStrangerCheckbox = (allowConnections) => {
    const checkboxCheckImage = document.getElementById("allow_strangers_checkbox_img"); 

    allowConnections? showElement(checkboxCheckImage): 
        hideElement(checkboxCheckImage);
};

export const showNoStrangersAvailableDialog = () => {
    const infoDialog = elements.getInfoDialog("No strangers available", "Please try again later");
